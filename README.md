...
|---------|-------------|----------------|
| `CC_` Capability Contract | Maps/validates HTTP requests; maps execution results to HTTP responses | `pgs_ingress/` |
| `EV_` Event | Emitted at egress boundary | `pgs_egress/` |
| `RB_` Runtime Binding | Connects transport CCs to their HTTP gateway implementation | `pgs_ingress/` |
